...
define systemd::resolved::dropin_file (
  Systemd::Dropin                             $filename                = $name,
  Enum['present', 'absent', 'file']           $ensure                  = 'present',
  Stdlib::Absolutepath                        $path                    = '/etc/systemd/resolved.conf.d',
  ...
  Boolean                                     $notify_resolved         = true,
) {
  include systemd

  if $systemd::manage_resolved == false {
    fail('systemd::resolved::dropin_file is disabled because systemd::manage_resolved is set to false')
  }
    ...
    ensure_resource('file', $path,
      {
        ...
        owner                   => 'root',
        group                   => 'root',
        recurse                 => $systemd::resolved_purge_dropin_dirs,
        purge                   => $systemd::resolved_purge_dropin_dirs,
  ...
  if $notify_resolved {
    File[$full_filename] ~> Service['systemd-resolved']
  }
}
